...
class ControladorFilial:
    ...
    def modificar_dados(self):
        ...
        # escolheu um atributo para modificar
            ...
            # atualiza os contratos apos qualquer uma das modificacoes
            contrato_gerente = self.__controlador_contrato.contrato_dao.get(self.__filial.gerente.cpf)
            contrato_gerente.filial = self.__filial
            contratos_a_atualizar = [contrato_gerente]
            for _ in self.__filial.funcionarios:
                contrato = self.__controlador_contrato.contrato_dao.get(_.cpf)
                contrato.filial = self.__filial
                contratos_a_atualizar.append(contrato)
            for _ in contratos_a_atualizar:
                self.__controlador_contrato.contrato_dao.update(_)
